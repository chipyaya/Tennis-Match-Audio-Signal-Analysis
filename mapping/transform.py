...
if __name__ == '__main__':
    img_path_format = 'video2image/us19-images/{0:03d}.png'
    start, end = 1, 7

    # get baseline and sideline
    for i in range(start, end):
        tf, baseline = getfb(img_path_format.format(i), baseline=True)
        if tf == True:
            break
        else:
            baseline = -1
    for i in range(start, end):
        tf, ftline = getfb(img_path_format.format(i), baseline=False)
        if tf == True:
            break
        else:
            ftline = -1
    ...
    for i in range(start, end):
        if getfb(img_path_format.format(i), baseline=True) == True:
            _, baseline = getfb(img_path_format.format(i), baseline=True) 
        if getfb(img_path_format.format(i), baseline=False) == True:
            _, ftline = getfb(img_path_format.format(i), baseline=False)  
        M = getMatrix(img_path_format.format(i), i, baseline, ftline)
